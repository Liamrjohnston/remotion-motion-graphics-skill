...
## Research and authentic assets before code

Named-product work now runs a compact, executable research preflight before concept selection:

- Exact user-supplied assets always win.
- A bundled local library provides ready-to-copy Claude, Codex/OpenAI, DeepSeek, Gemini, GitHub, Slack, Notion, Linear, Cursor, and Playwright marks.
- Missing identity is researched from 1–3 authoritative sources, preferring official transparent SVGs.
- Product UI/dashboard work requires a supplied or authoritative real surface. The agent cannot invent a generic SaaS dashboard.
- The agent selects the closest approved reference by story mechanism, while still allowing a better request-specific concept.
- Research briefs, sources, and asset hashes are cached. Repeated work returns `RESEARCH CACHE HIT` instead of spending tokens rereading the same pages.

Unsafe SVGs, search-result thumbnails, approximate logo redraws, baked checker/gray backgrounds, hotlinked assets, fabricated UI, and unsupported metrics are blocked.
...
| --- | --- | --- |
| [`motion-graphics`](skills/motion-graphics/SKILL.md) | Gate | Research, authentic logos/product surfaces, reference selection, art direction, evidence, independent criticism, delivery authorization |
...
The foundation ships as an executable, fail-closed workflow—not taste advice the agent can skim. Before code, `scripts/promptible-gate.mjs` validates research sources, safely checks and hashes local logos/product surfaces, verifies the bundled approved references by SHA-256, requires written evidence, and issues `BUILD AUTHORIZED`. After rendering, the gate hashes the candidate MP4, contact sheet, and probe evidence; only a separate critic can issue the scores needed for `DELIVERY AUTHORIZED`.
...
The agent handles the rest: it researches only the named products and relevant surfaces, reuses cached work, selects and reviews the closest approved mechanism, builds the scene, typechecks, renders QA evidence, and submits that evidence to an independent critic. A failed or incomplete gate cannot be delivered.
...
2. **Cached research preflight** — use supplied/bundled authentic logos, inspect only the authoritative product evidence needed, require a real surface for product UI, and hash the results for reuse.
3. **Executable reference preflight** — automatically choose and verify the closest approved motion mechanism, inspect its actual MP4/contact sheet, and permit a more specific new concept when its inherited motion logic is documented.
4. **Build** — use real identity and researched product structure; no receipt, invoice, dossier, barcode, stamp, fake dashboard, or vibe-coded fallback.
5. **Candidate evidence** — hash the rendered MP4, contact sheet, and `ffprobe` metadata, then lock them while review is pending.
6. **Independent visual critic** — a separate reviewer scores research fidelity, reference fidelity, composition, identity, legibility, motion, and restraint. Any hard failure, any score below 8, or an average below 8.5 rejects delivery.
7. **Delivery authorization** — only evidence that passes the executable gate can be shown as the finished render.
...
skills/
├── motion-graphics/      # Research/assets/reference/critic gate + bundled logos/media
